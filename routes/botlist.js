...
const data = Object.fromEntries(
  Object.entries(bots).sort((a, b) => b[1].v - a[1].v)
);
...
async function parse(ids, w = "") {
    ...
    ids.map(async (item) => {
      try {
        ...
        const fields = {
          ...
          createdDate: () =>
            new Date(user.createdAt).toLocaleString().match(/^(.*?),/)[1],
          createdTime: () =>
            new Date(user.createdAt).toLocaleString().match(/,\s(.*)$/)[1],
          banner: () =>
            `https://autumn.revolt.chat/backgrounds/${profile.background._id}/${profile.background.filename}`,
          bio: () => profile.content,
        ...
        try {
          const b = JSON.parse(
            JSON.stringify(await client.bots.fetchPublic(item))
          );
          object[item] = {
            bio: b.description,
            username: b.username,
  ...
  return Object.fromEntries(
    Object.entries(object).sort((a, b) => b[1].votes - a[1].votes)
  );
}
...
router.get("/random", async (req, res) => {
  ...
  if (amount > keys.length) {
    amount = keys.length; // Limit amount to the total number of available dictionaries
  }
...
router.get("/tag/:tag", async (req, res) => {
  ...
  const result = await parse(
    Object.keys(data).filter((id) => data[id].t.includes(tag))
  );
